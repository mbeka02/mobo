...
// Store wraps the database connection pool and generated queries.
type Store struct {
	db *pgxpool.Pool
	*dbgen.Queries
}
...
func NewStore(ctx context.Context, cfg config.DatabaseConfig) (*Store, error) {
	storeOnce.Do(func() {
		...
		storeInstance = &Store{db: pool, Queries: dbgen.New(pool)}
	})
...
// ExecTx executes queries within a database transaction.
