...
func (maker *JWTMaker) Create(userId uuid.UUID, email, role string, tokenType TokenType, duration time.Duration) (string, error) {
	payload := NewPayload(userId, email, role, tokenType, duration)
